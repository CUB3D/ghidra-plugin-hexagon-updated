...
//		}
//		buf.write(PCodeEmitPackedRevived.end_tag);
//	}
//
//	void dump(PackedBytes buf, int opcode, Varnode[] in, int isize, Varnode out) {
//		buf.write(PCodeEmitPackedRevived.op_tag);
//		buf.write(opcode + 0x20);
//		if (out == null) {
//			buf.write(PCodeEmitPackedRevived.void_tag);
//		} else {
...
//		buf.write(PCodeEmitPackedRevived.spaceid_tag);
...
//		buf.write(PCodeEmitPackedRevived.addrsz_tag);
...
//		buf.write(PCodeEmitPackedRevived.inst_tag);
...
//		enc.writeSignedInteger(ATTRIB_OFFSET, minAddr.getOffset());
//		buf.write(PCodeEmitPackedRevived.inst_tag);
//		writeOffset(buf, packetSize);
...
//		}
//
//		enc.closeElement(ElementId.ELEM_INST);
...
//		buf.write(PCodeEmitPackedRevived.end_tag);
	...
	List<PcodeOp> fixupPcode(Instruction instr, boolean part1, boolean part2) throws UnknownInstructionException {
		...
		ops.addAll(Arrays.asList(proto.getPcode(ctx, null)));
	...
	void recordBranchesAndValidateImplementedPcode(InstructionIterator insnIter, UniqueAddressFactoryRevived uniqueFactory)
		...
		while (insnIter.hasNext()) {
			...
			// ourselves
			PcodeOp[] ops = insn.getPrototype().getPcode(insn.getInstructionContext(), null);
	...
	UniqueAddressFactoryRevived uniqueFactory;

	public List<PcodeOp> getPcode(InstructionContext context, UniqueAddressFactoryRevived uniqueFactory)
			throws UnknownInstructionException {
	...
	public void getPcodePacked(InstructionContext context, PackedEncode enc, UniqueAddressFactoryRevived uniqueFactory)
			throws UnknownInstructionException {
		try {
